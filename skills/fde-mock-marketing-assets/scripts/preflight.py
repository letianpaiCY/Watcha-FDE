...
LOGIN_COMMAND = "lark-cli auth login --domain docs --domain drive --domain base"
...
def main() -> int:
    ...
    if version.returncode != 0:
        result.update({"status": "broken_lark_cli", "message": "飞书 CLI 已存在但无法运行，请重新安装后再试。"})
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    ...
    if verified or token_status in {"valid", "active"}:
        result.update({"ready": True, "status": "ready", "message": "飞书 CLI 和用户登录状态正常。"})
    else:
        result.update({"status": "needs_auth", "message": "飞书用户身份未通过验证，请完成登录授权。"})
    print(json.dumps(result, ensure_ascii=False, indent=2))
    return 0
