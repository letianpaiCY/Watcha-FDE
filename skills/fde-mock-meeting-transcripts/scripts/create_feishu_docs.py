#!/usr/bin/env python3
"""Create five mock meeting transcripts as Feishu documents."""
...
def main() -> int:
    ...
    if len(assets) != 5:
        raise SystemExit(f"expected 5 transcripts in {ASSET_DIR}, found {len(assets)}")
    ...
    try:
        for title, body in documents:
            final_title = f"[FDE Mock·会议·{batch}] {title}"
            created.append(create_document(final_title, body))
        index_lines = [
            "本批次包含 5 份脱敏仿真企业会议字幕。",
            "",
            "字幕仅作为 FDE 共学营业务材料，不对应任何真实企业、人员或客户。",
            "",
            "## 会议目录",
            "",
        ]
